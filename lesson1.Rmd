...
The image below should look close to what you have when you launch RStudio. The `Console` command line is on the left, the `Environment` windows upper left and the `Navigator and Preview` tabs in lower right. This configuration can be modified later.
...
Perform the following exercises with the Old Faitful data set in your Lesson 1 script so you have the code for reference. Use `help(faithful)` in the console to bring up info about the data set. You can reveal the code and results with the buttons below, but try without the hints first.
...
4. Make a linear model and scatterplot of waiting time on the x axis and eruptions on the y axis using the `plot()` and `lm()` and `abline()` functions. Is there a relationship between wait time and the length of eruption?
...
1. Load the `cars` data set and assign it to an object called `cars`.
...
3. Make a linear model and scatterplot with best-fit line of the `carsMPG` data frame of weight on the x axis and miles per gallon on the y axis. Is there a relationship between weight and mpg?
